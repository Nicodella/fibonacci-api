...
public class FibonacciController {
    ...
    public ResponseEntity<Integer> getAccessCount(@PathVariable int n) {
        if (n <= 0) {
            return ResponseEntity.badRequest().body(Integer.valueOf(-1)); 
        }
        int cant = service.getCantAccesos(n);
